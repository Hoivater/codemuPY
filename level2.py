...
'''31 С помощью цикла заполните список целыми числами от 1 до 10.'''
listA = [i for i in range(1,11)]
print (listA)

'''32 Дана строка с буквами. Проверьте, что в этой строке не более двух заглавных букв.'''

stringA:str = 'efDrgEgrgrFtgrtrrgf'
listA:List[str] = [i for i in list(stringA) if not i.islower()]
if len(listA) > 2: print(f'строка содержит более двух заглавных букв: {listA}')
else: print(f'строка содержит не более двух заглавных букв: {listA}')


'''33 Дан список со строками, содержащими целые числа:
...
def stringToNumeric(listTransform: List[str]) -> List[int]:
	listResult = [int(i) for i in listTransform]
	return listResult

print (stringToNumeric(listA))

'''34 С помощью цикла заполните список четными числами из промежутка от 1 до 100.'''

print([i for i in range(1, 101) if i%2==0])

'''35 ан список с числами:

[1, 2, 3, 4, 5]
Выведите в консоль элементы этого списка в обратном порядке.'''
listA = [1, 2, 3, 4, 5]
print([i for i in reversed(listA)])
...
а символы второй строки - значениями:
...
print(dict(zip(txt1, txt2)))

'''37 Дана строка с буквами и цифрами. Проверьте, что в этой строке не более трех букв.'''
stringA = 'dwd23у23431134145'
count = 0
for i in stringA:
	if not i.isdigit():
		count += 1
if count > 3: print(f'букв больше трех')
else: print(f'букв меньше или равно трем')


'''38 Дано число. Получите первую четную цифру с конца этого числа.'''

numA = 33513315
listA = [i for i in reversed(list(str(numA))) if int(i)%2==0]
try:
	print (f'последняя четная цифра числа {numA} равна: {listA[0]}')
except IndexError:
	print (f'четных цифр в числе {numA} - нет ')

'''39 Дана некоторая строка:

'abcde abcde abcde'
Замените в ней первый символ каждого слова на '!':

'!bcde !bcde !bcde'''
stringA = 'abcde abcde abcde'
#решение 1
stringN = stringA.replace('a', '!')
print (stringN)
#решение 2
listN = ['!' + i[1:] for i in stringA.split(" ")] 
print (" ".join(listN))


'''40 Дан список со строками, содержащими целые числа:
